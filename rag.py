...
def answer_with_rag(question: str, k: int = 8) -> tuple[str, pd.DataFrame]:
    ...
    prompt = (
        ...
        "excerpts support them: their actual name, then email address and "
        "phone number directly under the name when available, current "
        "employer, years of relevant experience stated as an explicit number "
        "(e.g. '~12 years' - compute from employment dates when not stated "
        "directly), and key skills. Never invent specifics not grounded in "
        "the excerpts.\n\n"
